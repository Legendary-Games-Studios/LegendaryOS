        ...
        for base in [
            ...
            for file in os.listdir(base):
                ...
                try:
                    ...
                    self.apps.append({
                        "id": file[:-3],  # Target ID derived from filename
...
class LegendaryOS(BoxLayout):
    ...
    # ================= BULLETPROOF APP CROSS-LAUNCH =================
    def launch_app_by_id(self, app_id, file_to_open=None):
        """
        Allows child apps (like Files) to directly request another app launch 
        by providing its exact filename identifier.
        """
        target_app = None
        for app in self.loader.apps:
            if app["id"] == app_id:
        ...
        if target_app:
            ...
            # Inject payload path directly into target app's context space
            if file_to_open:
                self.app_memory[app_id]["target_file"] = file_to_open
            
            Clock.schedule_once(lambda dt: self.launch_app(target_app), 0.05)
        else:
            # Fallback graphic popup if file mismatch or missing app occurs
